...
PLAN = [
    ...
    # nisa_gdx: 2026-04-24 実行済（PORTFOLIO に移管）
    {
        ...
        "amount": 700000, "label": "スポット予備枠（S&P500 押し目買い・2026-05-19 増額）", "priority": 5,
        # 2026-05-31: 発動条件を「底打ち or WTI$90以下」→「S&P500 高値比-10%以上 下落」に変更。
        # 理由: この枠が買う対象は S&P500 なのに、旧条件は WTI（原油）が安いと点灯し、
        # S&P500 が高値圏でも『買い』ランプが出る空振りが起きていた（WTI安≠S&P500安）。
        # ランプ＝本当にS&P500の買い時、にするため発動の核を S&P500 の下落率に一本化。WTIは参考表示に格下げ。
        "condition": {"type": "sp500_from_high", "value": -10},
        "condition_text": "S&P500 が高値比 -10%以上 下落（押し目買い）",
        "stage": "reserve",
        "note": "2026-05-19 月次積立化に伴い 20万→70万に増額。"
                "2026-05-31 発動条件を S&P500 高値比-10% に一本化（旧: 底打ち or WTI$90以下）。"
                "ランプ点灯＝S&P500が実際に-10%下落した時のみ＝迷わず投入検討できる。"
                "ヒステリシス: -10%発動 → -7%まで戻ると解除（HYSTERESIS_BUFFER sp500_from_high=3）。"
                "大暴落（Score 20以下）時の最後の砦。分割投入推奨（30+30+10 等）。貯金250万と組み合わせて使う。"
                "※ WTI・底打ちシグナルは別パネルで参考表示は継続。",
    },
